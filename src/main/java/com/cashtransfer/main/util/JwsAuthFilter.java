...
public class JwsAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        username = jwtUtil.extractUsername(jwt);
                System.out.println("JwsAuthFilter: JWT extracted: " + jwt.substring(0, Math.min(jwt.length(), 20)) + "..."); // Print first 20 chars
                            System.out.println("JwsAuthFilter: Username extracted from JWT: " + username);

        
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            var userDetails = this.userDetailsService.loadUserByUsername(username);
                        System.out.println("JwsAuthFilter: Username is valid (" + username + ") and SecurityContext is null. Attempting to load UserDetails.");
